    ...
    Context "Database import" {
        ...
        It "Should configure file-backed DuckDB connections to spill large operations to disk" {
            ...
            try {
                ...
                $settings = Invoke-DatabaseQuery -Database $db -Sql "select name, value from duckdb_settings() where name in ('memory_limit', 'temp_directory', 'preserve_insertion_order', 'threads');"
                $settingsByName = @{}
                foreach ($setting in $settings) {
                    $settingsByName[$setting.name] = $setting.value
                }

                $settingsByName['memory_limit'] | Should -Be '14.9 GiB'
