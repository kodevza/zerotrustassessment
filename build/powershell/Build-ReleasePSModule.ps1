...
$publicScripts = @(Get-ChildItem -Path (Join-Path $SourceDirectoryInfo.FullName 'public') -Recurse -Filter "*.ps1")
$functionNames = @($publicScripts.BaseName | Sort-Object)

Update-Metadata -Path $ModuleManifestFileInfo.FullName -PropertyName FunctionsToExport -Value $functionNames
Update-Metadata -Path $ModuleManifestFileInfo.FullName -PropertyName ModuleVersion -Value $moduleVersion
Update-Metadata -Path $ModuleManifestFileInfo.FullName -PropertyName Prerelease -Value $prerelease
...
Write-Host "Release module built successfully: $($OutputModuleManifestFileInfo.FullName)" -ForegroundColor Green
## Sign Module
